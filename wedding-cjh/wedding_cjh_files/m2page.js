...
var resizePage = function () {
    ...
    $("div.bindarea").each(function (i, divarea) {
        ...
        if ($(divarea).attr("idx") == "2") {
            ...
            // ITC02(배경 이미지)가 있는 영역에서만 ITC01 동적 확장 적용
            // 커버(이름) 영역처럼 ITC02가 없는 영역은 고정 높이 유지
            if ($(divarea).children("div.item.ITC02").length > 0) {
                var itc01Items = [];
                $(divarea).children("div.item.ITC01").each(function () {
                    itc01Items.push(this);
                });
                itc01Items.sort(function (a, b) {
                    return parseFloat($(a).css("top")) - parseFloat($(b).css("top"));
                });

                // top이 거의 같은 항목은 같은 행으로 묶어 동일 오프셋 적용.
                // (좌우로 나란히 배치된 신랑/신부 이름이 서로 밀리지 않도록)
                var rowThreshold = 5;
                var k = 0;
                while (k < itc01Items.length) {
                    var rowTop = parseFloat($(itc01Items[k]).css("top"));
                    var rowOverflow = 0;

                    while (k < itc01Items.length &&
                           Math.abs(parseFloat($(itc01Items[k]).css("top")) - rowTop) < rowThreshold) {
                        var el = itc01Items[k];
                        var curTop = parseFloat($(el).css("top"));
                        var curH = parseFloat($(el).css("height"));
                        var renderedH = $(el).children(".text").outerHeight(true);
                        var newH = Math.max(curH, renderedH);

                        // 같은 행은 누적 오프셋만 적용(서로 밀지 않음)
                        $(el).css("top", (curTop + totalOffset) + "px");
                        $(el).css("height", newH + "px");

                        // 행의 초과 높이는 가장 큰 값 1회만 다음 행으로 누적
                        rowOverflow = Math.max(rowOverflow, newH - curH);
                        k++;
                    }

                    totalOffset += Math.max(0, rowOverflow);
                }
            }

            $(divarea).css("height", areaHeight * ratio + totalOffset + "px");
        }
